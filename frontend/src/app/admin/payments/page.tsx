...
export default function AdminPayments() {
  const payments = [
    { id: "1", tenant: "John Doe", unit: "A1", amount: 1200, fee: 5, total: 1205, status: "Paid", date: "2026-04-01", method: "ACH" },
    { id: "2", tenant: "Jane Smith", unit: "B4", amount: 1200, fee: 35.10, total: 1235.10, status: "Paid", date: "2026-04-02", method: "Card" },
    { id: "3", tenant: "Robert Johnson", unit: "1", amount: 1200, fee: 0, total: 1200, status: "Overdue", date: "2026-04-01", method: "-" },
  ]
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold font-heading tracking-tight text-foreground">Payments</h1>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-border bg-card shadow-sm hover:shadow-md transition-all rounded-3xl group relative overflow-hidden">
          ...
          <CardContent className="relative z-10">
            ...
            <div className="absolute right-0 top-0 w-24 h-24 bg-green-500/10 rounded-full -mr-12 -mt-12 transition-all group-hover:scale-110" />
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card shadow-sm hover:shadow-md transition-all rounded-3xl group relative overflow-hidden">
          ...
          <CardContent className="relative z-10">
            ...
            <div className="absolute right-0 top-0 w-24 h-24 bg-destructive/10 rounded-full -mr-12 -mt-12 transition-all group-hover:scale-110" />
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card shadow-sm hover:shadow-md transition-all rounded-3xl group relative overflow-hidden">
          ...
          <CardContent className="relative z-10">
            ...
            <div className="absolute right-0 top-0 w-24 h-24 bg-accent/10 rounded-full -mr-12 -mt-12 transition-all group-hover:scale-110" />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-md group">
          ...
          <Input className="pl-12 h-12 rounded-2xl border-border bg-card shadow-sm focus:border-primary transition-all font-medium" placeholder="Search payments..." />
        </div>
      </div>
            ...
            {payments.map((p) => (
              <TableRow key={p.id} className="hover:bg-secondary/30 transition-colors border-border">
                ...
                <TableCell className="py-4 text-right">
                  <Button variant="outline" size="sm" className="rounded-lg border-border hover:bg-secondary text-[10px] font-bold uppercase tracking-widest font-heading transition-all">PDF</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
