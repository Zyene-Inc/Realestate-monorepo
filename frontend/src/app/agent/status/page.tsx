...
export default function AgentStatusPage() {
  ...
  if (isLoading)
    return (
      <main
        id="main-content"
        className="mx-auto min-h-[100dvh] max-w-3xl px-5 py-16"
      >
        <Skeleton className="h-10 w-48" />
        <Skeleton className="mt-12 h-72 w-full rounded-[1.5rem]" />
      </main>
    );
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              Use the account connected to your company application.
            </p>
            <a
  ...
  const tone = declined
    ...
    : approved
      ...
      : "text-warning-foreground bg-warning/18";

  return (
    <div className="min-h-[100dvh] bg-background">
      <header className="border-b border-border bg-card/90 py-3 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-5 sm:px-8">
          <Logo className="h-9" />
      ...
      >
        <section className="grid gap-6 border-b border-border pb-8 lg:grid-cols-[.72fr_1.28fr] lg:items-center">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              {approved
                ? "You can now create sale listings, send them for review, and respond to buyer questions from the company workspace."
                : declined
                  ? profile?.declineReason ||
                    "Review the company information below and contact our team if you need more context."
                  : "The property team is reviewing your company details and documents. A decision will be sent to your email."}
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              {approved ? (
                <Link href="/agent/listings" className={buttonVariants()}>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[15rem_1fr]">
          <aside className="text-sm">
            ...
            <p className="mt-3 font-semibold">{profile?.companyName}</p>
              ...
              <Card className="mt-6 border-primary/25">
                <CardContent className="p-6">
                  ...
                  <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                    Update the company details and verification documents above,
                    then return the application to the review queue.
                  </p>
                  <Button
                    className="mt-5"
