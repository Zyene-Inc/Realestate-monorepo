      ...
      <main id="main-content">
        <section className="public-container grid min-h-[calc(100dvh-4.5rem)] items-center gap-10 py-10 lg:grid-cols-[minmax(22rem,.9fr)_minmax(0,1.1fr)] lg:gap-14 lg:py-12">
          <div className="max-w-xl lg:py-8">
            <p className="text-sm font-semibold text-primary">Missouri real estate, close to home</p>
            ...
            <p className="mt-7 max-w-lg text-base leading-7 text-muted-foreground sm:text-lg">
              Coach Johnson Realty guides Missouri buyers, sellers, renters, and owners with clear advice and accountable service.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button nativeButton={false} size="lg" render={<Link href="/properties" transitionTypes={["nav-forward"]} />}>
                Explore properties <ArrowRight aria-hidden="true" />
              </Button>
              <Button nativeButton={false} size="lg" variant="outline" render={<Link href="/contact" transitionTypes={["nav-forward"]} />}>
                Talk with our team
              </Button>
            </div>
        ...
        <section id="services" className="scroll-mt-24 border-y border-border bg-card py-20 sm:py-28">
          <div className="public-container grid gap-14 lg:grid-cols-[minmax(18rem,.68fr)_minmax(0,1.32fr)] lg:gap-20">
            <div className="lg:sticky lg:top-28 lg:self-start">
              <p className="text-sm font-semibold text-primary">One team, every property move</p>
              ...
              <p className="mt-5 max-w-md text-base leading-7 text-muted-foreground">
                Advice, representation, leasing, and management work better when the details live with people who know the property.
              </p>
            </div>
            ...
            <div className="grid border-t border-border">
              {services.map((service) => {
                ...
                return (
                  <article key={service.title} className="grid gap-5 border-b border-border py-8 sm:grid-cols-[3.25rem_minmax(10rem,.65fr)_minmax(0,1fr)] sm:items-start sm:gap-7">
                    <span className="flex size-12 items-center justify-center rounded-xl bg-secondary text-primary">
                      <Icon className="size-5" strokeWidth={1.7} aria-hidden="true" />
                    </span>
                    <h3 className="text-xl font-semibold tracking-[-0.025em]">{service.title}</h3>
                    <div>
                      <p className="max-w-lg text-sm leading-6 text-muted-foreground">{service.copy}</p>
                      <Link href={service.href} transitionTypes={["nav-forward"]} className="focus-ring mt-4 inline-flex items-center gap-2 rounded text-sm font-semibold text-foreground hover:text-primary">
                        {service.link} <ArrowUpRight className="size-4" aria-hidden="true" />
                      </Link>
                    </div>
                  </article>
                );
        ...
        <section className="py-20 sm:py-28">
          <div className="public-container grid gap-6 lg:grid-cols-[1.18fr_.82fr] lg:grid-rows-[auto_auto]">
            ...
            <div className="rounded-[1.5rem] border border-border bg-card p-7 sm:p-10 lg:p-12">
              <p className="text-sm font-semibold text-primary">Experience before the transaction</p>
              ...
              <p className="mt-5 text-base leading-7 text-muted-foreground">
                A good decision includes the home, the block, the upkeep, the terms, and what happens after the keys change hands.
              </p>
              <Button nativeButton={false} variant="outline" className="mt-7" render={<Link href="/about" transitionTypes={["nav-forward"]} />}>
            ...
            <div className="grid gap-5 rounded-[1.5rem] border border-border bg-secondary p-7 sm:grid-cols-[auto_1fr] sm:items-start sm:p-10 lg:grid-cols-1">
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-6 text-muted-foreground">Neighborhood context and practical property knowledge shape every recommendation.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="border-y border-border bg-card py-20 sm:py-28">
          <div className="public-container grid items-center gap-10 lg:grid-cols-[.82fr_1.18fr] lg:gap-20">
            <div>
              <p className="text-sm font-semibold text-primary">A capable team after closing day</p>
              ...
              <p className="mt-5 max-w-lg text-base leading-7 text-muted-foreground">
                Owners get accountable oversight. Residents get clear answers. Properties get the attention that protects their long-term value.
              </p>
              <Link href="/contact" transitionTypes={["nav-forward"]} className="focus-ring mt-7 inline-flex items-center gap-2 rounded text-sm font-semibold text-foreground hover:text-primary">
                Ask about property management <ArrowUpRight className="size-4" aria-hidden="true" />
              </Link>
            </div>
        ...
        <section id="portal-access" className="scroll-mt-24 py-20 sm:py-24">
          <div className="public-container grid gap-9 rounded-[1.5rem] border border-border bg-secondary p-7 sm:p-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center lg:p-14">
            <div>
              <span className="flex size-12 items-center justify-center rounded-xl bg-card text-primary">
                <MessageSquareText className="size-5" strokeWidth={1.7} aria-hidden="true" />
              </span>
              <h2 className="mt-5 text-3xl font-semibold tracking-[-0.04em]">Already working with us?</h2>
              <p className="mt-3 max-w-md text-sm leading-6 text-muted-foreground">Choose the secure workspace that matches your relationship with Coach Johnson Realty.</p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ["Resident portal", "/tenant/login", "Lease, payments, and service"],
                ["Agent workspace", "/agent/login", "Listings and inquiries"],
                ["Staff access", "/admin/login", "Property operations"],
              ].map(([title, href, description]) => (
                <Link key={title} href={href} transitionTypes={["nav-forward"]} className="focus-ring group rounded-[1.25rem] border border-border bg-card p-5 transition-[border-color,transform] duration-200 hover:-translate-y-1 hover:border-primary/35">
                  <p className="font-semibold text-foreground group-hover:text-primary">{title}</p>
                  <p className="mt-2 text-xs leading-5 text-muted-foreground">{description}</p>
                  <ArrowRight className="mt-5 size-4 text-primary" aria-hidden="true" />
                </Link>
              ))}
            </div>
          </div>
