...
const items = [
  ...
  { title: "Units", icon: DoorOpen, href: "/admin/units" },
  { title: "Tenants", icon: Users, href: "/admin/tenants" },
  { title: "Leases", icon: FileText, href: "/admin/leases" },
  { title: "Payments", icon: CreditCard, href: "/admin/payments" },
  { title: "Maintenance", icon: Wrench, href: "/admin/maintenance" },
  { title: "Vendors", icon: UserSquare2, href: "/admin/vendors" },
        ...
        <div className="p-3 bg-secondary/50 rounded-2xl border border-border group hover:border-accent/30 transition-all">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-bold text-foreground truncate font-heading">{user?.email?.split('@')[0] || "Admin User"}</p>
              <p className="text-[10px] text-muted-foreground truncate uppercase tracking-widest font-bold font-heading">
                {user?.role === 'SUPER_ADMIN' ? 'Super Admin' : 'Property Manager'}
              </p>
            </div>
            <button 
              onClick={logout}
