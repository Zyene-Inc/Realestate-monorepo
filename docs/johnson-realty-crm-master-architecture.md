...
Weighted the same way, the current baseline capability footprint remains **93.2%** until the reviewed production migration is applied. Generic tenant-document management and acknowledgement/notification expansion are now code-complete and covered by focused tests, but they remain deployment-gated on that migration; the only continuing product/provider partial is e-signature activation.
...
- `announcements` now supports tenant and approved-agent audiences, optional required acknowledgements, scoped feeds, acknowledgment audit events, and per-recipient notification creation. `notifications` now provides a role-protected inbox with individual/read-all actions and deep links. Status: **CODE COMPLETE — production migration pending**.
...
- `documents` now supports private, tenant-scoped generic document records with server-authorized signed upload/download URLs, type/size enforcement, ownership-safe deletion, audit events, tenant UI, and Rental/Super Admin records UI. Status: **CODE COMPLETE — production migration pending**.
...
- Public sale/rental properties, the complete rental-application workspace, Phase 6 rental-admin screens, Phase 10 Super Admin reporting, vendors, announcements, notifications, tenant documents, and tenant-initiated payment screens use backend data. The last two await the reviewed production migration before deployment.
...
| Tenant                                    | **CODE COMPLETE — deployment-gated** — invited-only authentication, active lease/dashboard, maintenance with photos, direct management messaging, scoped notices/acknowledgements, notification inbox, and private documents are implemented. Tenant documents, notifications, and acknowledgements require the reviewed production migration before deployment. |
...
| Tenant documents, chat, announcements, and payments end to end | **CODE COMPLETE — deployment-gated**                                                                                                                                   | Tenant chat, announcements, notifications, acknowledgment handling, private document management, and tenant-initiated one-time rent payments are implemented. The document/notification/acknowledgment database migration must be applied before deployment; automatic debit is intentionally excluded.                                                                                                                                                                                                       |
...
| Announcement acknowledgments and agent-wide notices | **CODE COMPLETE — deployment-gated** — approved agents receive agent-audience notices; required tenant/agent acknowledgements are persisted and audited. The reviewed production migration is still required. |
...
| Tenant-facing portal                                                         | **CODE COMPLETE — deployment-gated** — lease, maintenance, messages, announcements, notification inbox, acknowledgements, private documents, and tenant-initiated rent collection are implemented. The reviewed production migration is required before the newly completed portal features deploy. |
...
| Documents, storage           | **CODE COMPLETE — deployment-gated**            | Generic tenant-document APIs/UI use private Supabase Storage, server-authorized signed upload/download URLs, type/size checks, tenant scope, ownership-safe deletion, and audit events. The reviewed production migration is still required.                                                                                          |
...
| Announcements, notifications | **CODE COMPLETE — deployment-gated**            | Admins can send scoped tenant or approved-agent announcements, request acknowledgements, and create recipient notifications. Tenant and agent inbox/feed UIs implement read and acknowledgement workflows; the reviewed production migration is still required.                                                                       |
...
### Phase 12 — Public Website Property Assistant — **PRODUCTION CONFIGURATION VERIFIED; END-TO-END BROWSER CHECK PENDING**
...
- **VERIFIED PRODUCTION STATE** — `ChatConversation`, `ChatMessage`, `ChatMessageRole`, the daily expiry cron, the production feature flag, and the API availability endpoint are present. The local chatbot migration source is absent from Supabase migration history, so it is a documented history exception and must not be replayed solely to repair that ledger.

Phase 12 implementation score: **4 / 4**. A controlled streamed-reply, refresh-history, role-exclusion, and Supabase-advisor browser check remains to be recorded before calling the public flow fully verified. This optional phase is outside the original 11-phase weighted score.
